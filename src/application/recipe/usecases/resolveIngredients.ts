...
import type { RecipeFormData, RecipeFormIngredient, MealieFood, MealieUnit } from "../../../shared/types/mealie.ts"
...
export async function resolveIngredients(
  ...
  const [allFoods, allUnits] = await Promise.all([
    needsFoodResolution ? foodRepository.getAll() : Promise.resolve<MealieFood[]>([]),
    needsUnitResolution ? unitRepository.getAll() : Promise.resolve<MealieUnit[]>([]),
  ])

  const foodCache = new Map<string, MealieFood>()
  for (const f of allFoods) {
    foodCache.set(f.name.toLowerCase().trim(), f)
  }

  const unitCache = new Map<string, MealieUnit>()
